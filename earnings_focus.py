...
def earnings_focus_exclusion_block(tickers: list[str]) -> str:
    ...
    return (
        ...
        "· **beat／miss**：僅當該則新聞 **title 或 summary** 同時含（1）對照語如「共識／預期／預估／華爾街／分析師預期／Street／consensus／estimate」與（2）結論語如 beat／miss／超預期／遜於預期 等時，正文方可使用賣方 beat/miss 口徑；否則僅能寫「季報數字已披露」。\n"
        "· **FY／前瞻指引**：僅能複述**同一則**新聞 title/summary 已寫之指引或展望；未寫則僅能「指引待法說／IR」。\n"
        ...
        "禁止臆測 EPS 數值或 beat/miss **幅度**；beat/miss 用語須符合上款「標題／摘要對照共識」檢查。\n"
