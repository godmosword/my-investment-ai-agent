...
_CRYPTO_PICK_KW: tuple[str, ...] = (
    ...
    # 技術／盤面讀值（須可對照區塊①；與鏈上、新聞並列為「線索」）
    "RSI", "MACD", "MA50", "MA20", "均線", "技術面", "超買", "超賣", "背離",
    "多頭排列", "空頭排列", "支撐", "壓力",
    # 宏觀外溢與鏈上估值讀值（對風險資產定價）
    "DXY", "美元指數", "VIX", "美債", "殖利率", "利差", "SOFR", "NVT", "Dominance",
)
...
_EQUITY_PICK_KW: tuple[str, ...] = (
    ...
    # AI 基礎設施／電力／供應鏈敘事（事件驅動型；排除泛稱以避免「基本面+電力+能源」等背景詞繞過懶惰敘事偵測）
    "核電",
    "SMR",
    "擴產",
    "供電",
...
def _pick_justification_crypto_ok(text: str, recs: list[dict]) -> tuple[bool, str]:
    ...
    ≥2 個催化/鏈上/技術盤面/宏觀讀值關鍵線索；或 1 線索 + 明確大型幣退階語；
    或 1 線索 + 長文且點名所有標的。
    """
    crypto_assets = [
        str(r.get("asset", ""))
    ...
    return (
        False,
        "本日選擇理由（加密）未達動態選幣標準：須（≥2 項催化/鏈上/技術盤面/宏觀讀值線索）或（1 線索+大型幣退階說明）或（1 線索+長文且點名所有加密 QSREC 標的）；不符則請改選標的或補強敘事",
    )
