...
### Ops（ITER-GCP-EXIT-P2 — API 上同一 Vercel 專案 polyglot）
...
- **正式切換（2026-09-09）**：GitHub secret `VITE_API_URL` 已清空；`pwa-deploy` prebuilt `dpl_3u3yKCGmgYZoY8zqVwFKg3Y3C7X5`（source=cli）。`GET /healthz` 精確 JSON。`BASE_URL=https://my-investment-ai-agent.vercel.app npm run smoke:prod` 全過（靜態五路 + liveness + quote 200 + data-health／options）。**不要**本機 `vercel --prod`。Function 除 `/tmp` 唯讀：意圖 PATCH／jsonl 不持久，待 P3。
...
- **CI prebuilt**：`deploy-vercel` 在 `vercel build` 前裝 [`astral-sh/setup-uv`](https://github.com/astral-sh/setup-uv)（GitHub runner 沒有 `uv`）。SPA 路徑曾 404：頂層 rewrite 進 PWA service 後，子目錄 `vercel.json` 不會自動合併，需 `services.pwa.rewrites` → `/index.html`。
...
- **文件**：[`docs/PORTAL_SHIP_CHECKLIST.md`](docs/PORTAL_SHIP_CHECKLIST.md)「2026-09-05 正式上線」— 三條定義、Job ≠ Service、正式 Service **503**／`/healthz` **404**、Human 看 `[REDACTED]-api`（`asia-east1`）revision／logs、`smoke:prod` 指令。
