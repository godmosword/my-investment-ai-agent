...
### Added
- **PWA 視覺化 V2（結構化 Report — 首批）**：[`api.py`](api.py) — `GET /api/reports/{report_date}/structured`（`profile` query，對齊 [`brief_profiles`](brief_profiles.py)；回傳 **`block_ids`**、**`block_registry`**、**`legacy`**、**`daily_brief_report`: null**、**`structured_body_available`: false**）；[`test_report_structured_api.py`](test_report_structured_api.py)。PWA — [`useStructuredReport`](data-verification-ui/src/hooks/useApi.js)、[`StructuredReportView.jsx`](data-verification-ui/src/components/report/StructuredReportView.jsx)、[`legacyBlockContent.js`](data-verification-ui/src/components/report/legacyBlockContent.js)；[`Report.jsx`](data-verification-ui/src/pages/Report.jsx) 以 **`VITE_STRUCTURED_REPORT=1`** 啟用（預設關閉仍走 legacy `GET /api/reports/{date}`）。契約／環境見 [`docs/DASHBOARD_CONTRACT.md`](docs/DASHBOARD_CONTRACT.md)、[`ENV_TEMPLATE.txt`](ENV_TEMPLATE.txt)。
