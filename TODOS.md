...
**同步狀態（2026-06-16 — `pwa-deploy` E2E 掛死防護）**：[`pwa-deploy.yml`](.github/workflows/pwa-deploy.yml) `verify`／`deploy-vercel` timeout（35／20 分）、`skip_e2e` 緊急部署、`playwright-report` artifact；Playwright **Docker 映像** `v1.59.1-jammy`（run #27772957639 拆分 install 仍解壓卡死後改）；[`pwa-e2e.yml`](.github/workflows/pwa-e2e.yml) 移除 main push 雙跑；deploy 對齊 Vercel **Output Directory** `dist/data-verification-ui`（run #27795573529／#27795682635）。本機 `CI=true npm run test:e2e` **86/86**（~29s）。**PR-3 spec 修補** 待 E2E 測試本身失敗且有 artifact 再開。見 CHANGELOG **2026-06-16**、[`docs/PORTAL_SHIP_CHECKLIST.md`](docs/PORTAL_SHIP_CHECKLIST.md)「CI timing and emergency deploy」。
