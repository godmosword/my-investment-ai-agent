...
29. **Portal Phase 2 — Command Bar + SSE 即時 Quote（週 1–2）** — **核心 Bloomberg 感體驗**；`/api/stream/war-room` SSE 推送 quote 事件（已有 endpoint，需補 symbol-specific push）；全局 Command Bar（`AAPL <GO>` 語意）路由至對應模組或觸發 Crew；`SymbolSnapshotCard` + 輕量 Candle Chart（lightweight-charts v5 已接）。**前置**：隊列 **26** `Shell + daily-brief` 完成；**紅線**：不新增未審核即時付費資料依賴；每切片附 Playwright smoke。P1 / M。
30. **M4（Portal）Position Management 完整管理（週 3–4）** — [`PositionsHome.jsx`](data-verification-ui/src/modules/position-management/pages/PositionsHome.jsx)（隊列 26 切片已有骨架）升級為**可用 MVP**：Portfolio 倉位表（open/closed）、Intent 表單（新增／修改 execution intent）、Risk Metrics 欄（size / stop / target）、Paper P&L 回放；後端補 `GET /api/positions` 聚合 + `PATCH` 更新；與 [`execution_intents.py`](execution_intents.py)／[`paper_execution.py`](paper_execution.py) 閉環。**前置**：隊列 28a paper P&L 信號鏈完成；每切片附 pytest + Playwright smoke。P1 / M。
31. **M5（Portal）Industry Trends 產業趨勢頁（週 5–6）** — `modules/industry-trends/` 新增 Theme 頁面 + 趨勢卡片（sector rotation、macro regime tag）；後端 `GET /api/industries/themes`（可先從 [`current_affairs_crew.py`](current_affairs_crew.py) 萃取 sector signal）；與 `agreed_regime`／`BRIEF_DYNAMIC_RENDER` 對齊；每切片附 Playwright smoke。P2 / M。
32. **M6（Portal）Investment Analysis 個股深度頁（週 7–8）** — `modules/investment-analysis/` Snapshot + Deep Dive + News + Filing Block；後端 `GET /api/analysis/{symbol}`（整合 [`symbol_snapshot_service.py`](symbol_snapshot_service.py) + `DeepFilingAnalysis` scaffold）；NotebookLM citation 區塊當 `NOTEBOOKLM_ENABLED=1` 時顯示（隊列 24 已有 scaffold）；AgencyResearch block 在 `AGENCY_RESEARCH_ENABLED=1` 時顯示（隊列 25 已有 scaffold）；每切片附 pytest + Playwright smoke。P1 / L。
33. **M7（Portal）Quant Trading 訊號列表 + 回測 UI（週 9–10）** — `modules/quant-trading/` Signal Table（QSREC confidence / regime tag）、簡易 Backtest UI（選訊號時間範圍 → paper P&L 曲線）、Intraday Monitor（SSE 事件驅動）；後端 `GET /api/quant/signals` + `GET /api/quant/backtest`（接 [`scripts/symbol_price_probe.py`](scripts/symbol_price_probe.py) 歷史資料）；**紅線**：不承諾收益、不自動下單；每切片附 pytest + Playwright smoke。P2 / L。
34. **Portal Phase 3 — 多視窗 + Alert + 個人化（週 11–12）** — 可拖拉 panel（react-grid-layout 或輕量實作）；Web Push Alert 與 war-room digest 語意閉環（T4b 完成後）；Portfolio Scenario 模擬（隊列 28d 情境壓測前端化）；個人化 workspace 匯出匯入（localStorage → 可選雲端）；Bloomberg 對齊 Phase 0 最後缺口（BLOOMBERG_ALIGNMENT.md §15）。**前置**：隊列 29–33 MVP 全部可用；合規文案可審計（28d 條件）。P2 / L。
