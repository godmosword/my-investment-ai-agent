...
| 主題 | 代表檔案／行為 |
|------|----------------|
| **日報區塊模組化** | **文件**：[`modularization_plan.md`](modularization_plan.md) Phase 1–5、**[產品與交付原則](modularization_plan.md#產品與交付原則)**。**Phase 1 首切片（2026-04-26）**：[`templates/telegram_report.j2`](templates/telegram_report.j2) + [`templates/blocks/`](templates/blocks/)（尾段 **`_footer_tail.j2`** 逐字對齊凍結基線）；[`report_render.py`](report_render.py) `build_telegram_jinja_env`／`telegram_render_context`；**合併門檻** smoke [`test_telegram_template_modularization.py`](test_telegram_template_modularization.py)。**Phase 2–5** 仍見隊列 **22** |
...
22. **日報區塊模組化（實作）** — ~~**Phase 1**（`templates/blocks/`、`telegram_report.j2` 組裝、**byte-identical** smoke）~~ **已交付（2026-04-26）** — 見 CHANGELOG **2026-04-26** `### Changed` 與 [`modularization_plan.md`](modularization_plan.md) Phase 1「合併門檻」。**仍待**：**Phase 2** `brief_profiles`／`BLOCK_REGISTRY`／`REPORT_PROFILE`／`lite`；**Phase 3** Gate profile-aware；**Phase 4** `crypto-only`／可選 YAML／BQ `profile`；**Phase 5** 【時事多觀點】。原則見 **[產品與交付原則](modularization_plan.md#產品與交付原則)**（過渡期 **production 固定 `full`／等價**、新版型先 staging）。
