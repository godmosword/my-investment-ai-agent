...
| 主題 | 代表檔案／行為 |
|------|----------------|
| **日報區塊模組化計畫（文件，2026-04-25 起；2026-04-26 補原則）** | [`modularization_plan.md`](modularization_plan.md) — Phase 1–5、短中長期、BLOCK_REGISTRY／YAML 可選；**[產品與交付原則](modularization_plan.md#產品與交付原則)**（過渡期不影響預設日報產出、完成後組織級客製）；**尚未**改 `telegram_report.j2`／`main.py`（見下一批隊列 **22**） |
...
22. **日報區塊模組化（實作）** — 依 [`modularization_plan.md`](modularization_plan.md)（含 **[產品與交付原則](modularization_plan.md#產品與交付原則)**：過渡期 **production 固定 `full`／等價**、新版型先 staging；完成後 **`REPORT_PROFILE`／Registry／可選 YAML`** 組織客製）自 **Phase 1**（`templates/blocks/` macro 切分、`full` 輸出等價）起逐步交付；**Phase 2** `brief_profiles`／`BLOCK_REGISTRY`／`REPORT_PROFILE`／`lite`；**Phase 3** Gate；**Phase 4** `crypto-only`／可選 YAML／BQ `profile`；**Phase 5** 【時事多觀點】。計畫全文已於 **2026-04-25** 寫入 CHANGELOG `### Docs`；**2026-04-26** 補交付原則（見 CHANGELOG 同日 `### Docs`）；完成各 Phase 後寫入 CHANGELOG `### Changed`／`### Added` 並更新本節。
