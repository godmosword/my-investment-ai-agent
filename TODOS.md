...
**同步狀態（2026-05-08）**：**Crew 結構化 JSON** — [`crew_output_parse.py`](crew_output_parse.py) **`repair_llm_json_text`** + **`kickoff_to_pydantic`**／**`parse_pydantic_from_llm_json_text`**；**第二層** **`kickoff_with_structured_fallback`**（**`crew.kickoff`** 先拋 **`json_invalid`** 時從例外鏈抽原始 JSON 再修復）；[`crew.py`](crew.py) **Crypto／AI** 已套用；測試 [`test_crew_output_parse.py`](test_crew_output_parse.py)。見 CHANGELOG **2026-05-08**。
