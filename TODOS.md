...
**同步狀態（2026-06-16 — `pwa-deploy` 全綠）**：[`pwa-deploy.yml`](.github/workflows/pwa-deploy.yml) **verify**（lint + E2E **86/86**，Playwright `v1.59.1-jammy`）+ **deploy-vercel**（prebuilt：`vercel pull`→`build`→`deploy --prebuilt`，`vercel@54.14.2`；CLI 自 **repo 根目錄**執行以對齊 Vercel Root Directory `data-verification-ui`）— run [**#27797244523**](https://github.com/godmosword/my-investment-ai-agent/actions/runs/27797244523) ✅；正式站 [`my-investment-ai-agent.vercel.app`](https://my-investment-ai-agent.vercel.app) 靜態路由 200。**PR-3 spec 修補** 待 E2E 測試本身失敗且有 artifact 再開。見 CHANGELOG **2026-06-16**、[`docs/PORTAL_SHIP_CHECKLIST.md`](docs/PORTAL_SHIP_CHECKLIST.md)。
