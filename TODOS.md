...
**同步狀態（2026-09-09 — ITER-GCP-EXIT-P2）**：根目錄 [`vercel.json`](vercel.json) polyglot；Dashboard Root Directory = `.`；GitHub secret `VITE_API_URL` 已清空。正式 prebuilt `dpl_3u3yKCGmgYZoY8zqVwFKg3Y3C7X5`（source=cli）：`GET /healthz` 精確 JSON、`BASE_URL=https://my-investment-ai-agent.vercel.app npm run smoke:prod` 全過。P3 Job／P4 刪 GCP 未做。見 CHANGELOG **2026-09-09**。
...
**同步狀態（2026-09-06 — ITER-API-HTML-001）**：`GET /api/reports/{date}/html` 兩個既有 500 修掉 — `jinja2.Markup` 改 `markupsafe.Markup`；`tojson` 對 pydantic qsrec 物件改走 `model_dump(mode="json")`（`DailyBriefReport` 拒空 qsrec，故每份合法報告都會撞到第二個，只修其一無法讓路由通）。新增一條渲染成功的契約測試；其餘 reports 行為與 OpenAPI 表未動。**不部署 Service**。見 CHANGELOG **2026-09-06**。
...
**同步狀態（2026-09-08 — ITER-TR-LOOP-FALSE-NEG-001）**：紙上對帳條對分頁截斷／未辨識 status 不再假陰性標「無紙上記錄」（改 UNKNOWN／truncated）；`PENDING_REVIEW`／`REJECTED`／`SUPERSEDED` 仍標「無紙上記錄」。見 CHANGELOG **2026-09-08**。
...
| 主題 | 代表檔案／行為 |
|------|----------------|
| **ITER-GCP-EXIT-P2 — Vercel polyglot repo（2026-09-09）** | 根 [`vercel.json`](vercel.json) PWA+FastAPI；空 `VITE_API_URL` 同源；`VERCEL=1` 預設 skip BQ。**Preview `/healthz` 已通**（Root Directory = `.`）。**正式切換仍須清空 `VITE_API_URL` + `pwa-deploy`**。CHANGELOG **2026-09-09**。 |
...
| **P4-44B～44P — Portal 繁中／UNKNOWN／觸控包（2026-09-03～09-05，2026-09-08 補記）** | #170–#184：儀表板／情境／警示／DeepDive／指令列／dock／實績／健康 chip。**不是**隊列 62 密度實作。剩餘 em dash 見候選板。CHANGELOG **2026-09-08**。 |
...
<a id="gcp-exit-queue-72"></a>

## GCP 退出（隊列 72）

72. **徹底剔除 GCP — 託管與寫入** — **P1 已交付（2026-09-09）**；**P2 repo + preview 活體已交付（2026-09-09）**（根 `vercel.json` services、Root Directory = `.`、preview `GET /healthz` 精確 JSON）。**剩餘 Human／本機**：清空 GitHub secret `VITE_API_URL` → `pwa-deploy.yml` prebuilt → `BASE_URL=<prod> npm run smoke:prod`。**尚未完成**：
    - **P2 正式切換**：secret 仍指向已死 Cloud Run；清空前確認 preview `/healthz` 仍是精確 JSON（已證）。不要本機 `vercel --prod`。
    ...
    **紅線**：不假裝正式 `/insights` 已通，直到正式 origin 同源 `GET /healthz` 精確 JSON。P1 / L。
...
- **2026-09-09（ITER-GCP-EXIT-P2）**：根 `vercel.json` polyglot、同源 PWA 契約、`VERCEL=1` 預設 skip BQ；Root Directory = `.`。GitHub secret `VITE_API_URL` 已清空；`pwa-deploy` 裝 `uv`；正式 `/healthz` 與 `smoke:prod` 已通（`dpl_3u3yKCGmgYZoY8zqVwFKg3Y3C7X5`）。SPA `/insights` 曾 404，已在 `services.pwa` 加 `index.html` fallback。見 [`CHANGELOG.md`](CHANGELOG.md) **2026-09-09** `### Ops（ITER-GCP-EXIT-P2）`。
...
- **2026-09-09（ITER-GCP-EXIT-P1）**：API 檔案優先讀路徑；隊列 **72** GCP 退出 P2–P4。見 [`CHANGELOG.md`](CHANGELOG.md) **2026-09-09**。
