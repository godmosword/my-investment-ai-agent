...
23. **Reviewer Loop（LangGraph Phase 3.5）** — [`docs/architecture/REVIEWER_LOOP_DESIGN.md`](docs/architecture/REVIEWER_LOOP_DESIGN.md)：實作 `python_validate_node` → `llm_reviewer_node` → `degrade_node` 反思迴圈；`graph/graph_state.py` 新增 `trade_candidates`／`review_issues`／`revision_count`／`review_history`／`trade_watch_final`／`degraded` 欄位；Layer 1 deterministic（ticker 存在性／方向一致性／數字合理性／重複標的／Slim Schema 欄位）、Layer 2 LLM 僅查 narrative 矛盾與幻覺標的；**hard cap=2**、所有節點異常路由至 END、降級保留最後版本 + `⚠️` 警示；BQ `reviewer_log`（[`docs/SQL/reviewer_log.sql`](docs/SQL/reviewer_log.sql) 待建）；`test_reviewer_loop.py` 涵蓋 pass／python fail／llm fail／hard cap；驗收 7 項見設計稿。**紅線**：reviewer 僅查邏輯、**不取代** `validate_report`／Telegram HTML 白名單。**依賴**：隊列 **8**（`USE_LANGGRAPH_ENGINE=1` 預設翻轉）。P1 / M。
