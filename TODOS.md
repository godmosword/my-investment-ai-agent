...
| 主題 | 代表檔案／行為 |
|------|----------------|
| **日報區塊模組化** | **文件**：[`modularization_plan.md`](modularization_plan.md) Phase 1–5、**[產品與交付原則](modularization_plan.md#產品與交付原則)**。**Phase 1（2026-04-26）**：[`templates/blocks/`](templates/blocks/) + **`_footer_tail`** 凍結基線；smoke [`test_telegram_template_modularization.py`](test_telegram_template_modularization.py)。**Phase 2（2026-04-27）**：[`brief_profiles.py`](brief_profiles.py)、[`templates/profiles/`](templates/profiles/) `telegram_full.j2`／`telegram_lite.j2`、`REPORT_PROFILE`（[`ENV_TEMPLATE.txt`](ENV_TEMPLATE.txt)）；[`report_render.py`](report_render.py) `render_telegram_daily_brief(..., profile=)`；[`test_brief_profiles.py`](test_brief_profiles.py)（**`full` byte-identical**）。**Phase 3–5** 仍見隊列 **22** |
...
22. **日報區塊模組化（實作）** — ~~**Phase 1**~~ **已交付（2026-04-26）**；~~**Phase 2**（`brief_profiles`、`BLOCK_REGISTRY`、`REPORT_PROFILE`、`lite`、`profiles/*.j2`；**`full` byte-identical**）~~ **已交付（2026-04-27）** — 見 CHANGELOG **2026-04-27** `### Changed` 與 [`modularization_plan.md`](modularization_plan.md) Phase 2 表註記。**仍待**：**Phase 3** Gate profile-aware；**Phase 4** `crypto-only`／可選 YAML／BQ `profile`；**Phase 5** 【時事多觀點】。原則見 **[產品與交付原則](modularization_plan.md#產品與交付原則)**（過渡期 **production 固定 `full`／等價**、`lite` 先 staging）。
