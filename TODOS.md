...
| **PWA 視覺化 V2／V3 前置**（結構化 Report + profile／錨點／layout 清單 + **逐區塊原生映射**） | `GET /api/reports/{date}/structured`（**`daily_brief_report`**／**`gate_summary`**／**`structured_source`**）、`GET /api/brief-layouts`（[`api.py`](api.py)）、[`test_report_structured_api.py`](test_report_structured_api.py)、[`test_brief_layouts_api.py`](test_brief_layouts_api.py)；環境 **`DAILY_BRIEF_JSON_DIR`**（[`ENV_TEMPLATE.txt`](ENV_TEMPLATE.txt)）；PWA **`VITE_STRUCTURED_REPORT`**、[`structuredBlockContent.js`](data-verification-ui/src/components/report/structuredBlockContent.js)、[`StructuredReportView.jsx`](data-verification-ui/src/components/report/StructuredReportView.jsx)（頁級／區塊 **`AsOfChip`**、**`GateStatusBadge`**、Gate 橫幅；**`DailyBriefReport`** 優先、legacy 後備）、[`Report.jsx`](data-verification-ui/src/pages/Report.jsx) **`?profile=`**；CHANGELOG **2026-04-18**。**仍待／加強**：可選 **BQ 持久化**與營運搜尋路徑（[`visualization_plan.md`](visualization_plan.md)）、結構化報告頁 **E2E 煙霧**（可選）。 |
