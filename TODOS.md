...
**同步狀態（2026-09-09 — ITER-GCP-EXIT-P2）**：根目錄 [`vercel.json`](vercel.json) polyglot；Dashboard Root Directory = `.`；正式 `GET /healthz` 已通（prebuilt `dpl_4W7LanSCy4qDz8GBgbhYVCLnLbYC`）。GitHub secret `VITE_API_URL` 已清空。**正式 `/insights` 曾 404** — PWA service 缺 SPA `index.html` fallback。P3 Job／P4 刪 GCP 未做。見 CHANGELOG **2026-09-09**。
...
- **2026-09-09（ITER-GCP-EXIT-P2）**：根 `vercel.json` polyglot、同源 PWA 契約、`VERCEL=1` 預設 skip BQ；Root Directory = `.`。GitHub secret `VITE_API_URL` 已清空；`pwa-deploy` 裝 `uv`；正式 `/healthz` 已通。SPA `/insights` 曾 404，改在 `services.pwa` 加 `index.html` fallback。見 [`CHANGELOG.md`](CHANGELOG.md) **2026-09-09** `### Ops（ITER-GCP-EXIT-P2）`。
