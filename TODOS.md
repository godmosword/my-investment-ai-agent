...
**同步狀態（2026-06-16 — `pwa-deploy` E2E 掛死防護）**：[`pwa-deploy.yml`](.github/workflows/pwa-deploy.yml) `verify`／`deploy-vercel` timeout（35／20 分）、`skip_e2e` 緊急部署、`playwright-report` artifact；Playwright **拆分** `install-deps`（5m）／`install chromium`（10m，cache hit 略過）修復 run #27770267116 install 卡死；[`pwa-e2e.yml`](.github/workflows/pwa-e2e.yml) 移除 main push 雙跑、同步拆分 install；[`playwright.config.js`](data-verification-ui/playwright.config.js) CI `globalTimeout`／`maxFailures`；[`run-ci.sh`](data-verification-ui/e2e/run-ci.sh) mock／preview fail-fast。本機 `CI=true npm run test:e2e` **86/86**（~29s）。**PR-3 spec 修補** 待 CI artifact 鎖定掛死 test 後再開。見 CHANGELOG **2026-06-16**、[`docs/PORTAL_SHIP_CHECKLIST.md`](docs/PORTAL_SHIP_CHECKLIST.md)「CI timing and emergency deploy」。
