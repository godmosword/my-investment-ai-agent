...
### Portal 切片 — Crypto on-chain + 翻譯短評（隊列 45 · P5-mock / P4 · 2026-05-16）

**P5-mock**：`/dashboard` 在 compute-memory 下方新增 [`OnchainMetricsPanel.jsx`](data-verification-ui/src/components/OnchainMetricsPanel.jsx) 三區：BTC 估值（MVRV-Z／Realized／Spot/Realized）、交易所淨流入（CEX／Binance／Coinbase）、永續資費（BTC／ETH）。資料源 = 本機 fixture [`data/onchain_metrics_mock.json`](data/onchain_metrics_mock.json)；後端 `GET /api/macro/onchain` 與 P2-mock 同模式（fixture-missing → `enabled: false`；`live` 需 fixture + `ONCHAIN_LIVE=1` 雙條件）。

**P4**：[`api_routers/news.py`](api_routers/news.py) `_normalize_item` 增 `commentary_zh`／`commentary_en` 兩欄 **passthrough**（零 LLM 呼叫；zh 預設 fallback 既有 `gemini_take`，en 無 fallback）。[`ColumnsHome.jsx`](data-verification-ui/src/modules/columns/pages/ColumnsHome.jsx) Deep Brief Panel 顯示中／EN toggle（兩段皆存在時才出現），保留 source URL + published_at 並標「短評為翻譯／轉述；非原文」。

### Portal 切片 — 算力／記憶體 dashboard（隊列 45 · P2-mock · 2026-05-16）

`/dashboard` macro indicators 下方新增 [`ComputeMemoryPanel.jsx`](data-verification-ui/src/components/ComputeMemoryPanel.jsx) 三區並列：**HBM/DRAM 現貨**、**Hyperscaler Capex**、**GPU 現貨（hourly）**。資料源 = 本機 fixture [`data/compute_memory_mock.json`](data/compute_memory_mock.json)（範本，數值預設 `null` + `TEMPLATE` 註記，避免假數字進產線）。後端：[`api_routers/macro.py`](api_routers/macro.py) 之 `GET /api/macro/compute-memory`，fixture 缺／JSON 無效時回 `enabled: false`；`live` flag 需 fixture + `COMPUTE_MEMORY_LIVE=1` **雙條件** 才會 true（P2-live 入接 TrendForce／CoreWeave／hyperscaler capex 前需先過 [governance 表](docs/REALTIME_DATA_SOURCES_GOVERNANCE.md)）。E2E：[`dashboard-compute-memory.spec.js`](data-verification-ui/e2e/dashboard-compute-memory.spec.js)。

### Portal 切片 — Portfolio TP/SL 計算機（隊列 45 · P1 · 2026-05-16）

`/portfolio` 頁面 KPI 卡下方新增 [`PortfolioRiskPanel.jsx`](data-verification-ui/src/components/PortfolioRiskPanel.jsx)：輸入 **帳戶總值 × 每筆風險 %**（持久化於 `localStorage["qsi_risk_budget_v1"]`，預設 1%）+ entry／stop／target → 即時輸出 R:R、% 至 stop／target、建議股數（`floor(預算 ÷ 每股風險)`）、名目部位與實際風險 $。輸入 symbol 後可一鍵套用 **ATR14 stop**（純前端計算，重用 [`useAnalysisBundle`](data-verification-ui/src/hooks/useApi.js) 之 `snapshot.price_series`）或「帶入最後收盤」。算完一鍵 **送入紙上 `PENDING_REVIEW`**（重用 [`useCreateExecutionIntent`](data-verification-ui/src/hooks/useApi.js) → `POST /api/execution-intents`），**後端零變更、不下單**。E2E：[`portfolio-tpsl.spec.js`](data-verification-ui/e2e/portfolio-tpsl.spec.js)。
...
與 [`Terminal_Master_Plan.md`](docs/architecture/Terminal_Master_Plan.md) **§0 Phase 4** 對齊：新聞／專欄與觀點／儀表／組合同一 Portal、不同密度。Gate 0 預設集中於 [`portalPhase4.js`](data-verification-ui/src/constants/portalPhase4.js)（主戰場 `/insights` + `/portfolio`、宏觀 `/dashboard`、讀者首屏避高密度表、`N=3`、`terminalToneKeep` 五項）。**44a／44b／44d** 已落（讀者／工作台導引條、Command Bar placeholder 分路）；**44c 融合層雙向 CTA（2026-05-16 續）** — `PORTAL_PHASE4_CTA` 文案表 + `newsContextHref`／`columnsContextHref`／`ctaWithSymbol`，`fusionDirection=bidirectional`；`InsightsHome` 反向 CTA、`SymbolDeepDive` `?focus={SYM}` 雙向 CTA、`NewsHome`／`ColumnsHome` 接 `?focus=` 過濾並顯示 focus badge。Playwright：[`phase4-ia-portal.spec.js`](data-verification-ui/e2e/phase4-ia-portal.spec.js)。詳見 [`CHANGELOG.md`](CHANGELOG.md) **2026-05-16**「PWA（隊列 44 · 44c）」與 [`TODOS.md`](TODOS.md) [「隊列 44」](TODOS.md#terminal-master-plan-phase4-queue-44)。
...
| PWA E2E（Playwright） | `cd data-verification-ui && npm run test:e2e`（內建 mock API + `VITE_E2E=1` 建置；Bloomberg §6 Dashboard vs `/insights` BTC 價；另含 Command Bar Crew HUD、Workspace 跨分頁、**Phase 4 IA** `phase4-ia-portal.spec.js` 等 spec） | 否（需下載 Chromium） |
