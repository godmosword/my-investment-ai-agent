...
Current operating level is L1. `L2A_ACTIVATION_STATUS` = `BLOCKED_BY_RUNTIME_AUTO_REVIEW`. `MERGE_AUTONOMY` = `DISABLED_AT_CURRENT_RUNTIME`. While L1, `AUTO_MERGE_ELIGIBLE` is always `FALSE` and Human is the merger. Human B standing weekly quota lets Director start one qualifying board row after Routine F; it does not let routines implement and does not let anyone merge. Dormant L2A design stays in `.grok/TEAM_CHARTER.md` and must not self-restore. `HOLD_FOR_HUMAN` remains required for R3, production coupling, and any Human HOLD. It is not an orchestration failure.
