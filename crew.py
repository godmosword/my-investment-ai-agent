...
from assets_universe import (
    ...
    equity_core_tickers,
    financial_datasets_tickers_for_prompt,
)
from tools import (
    ai_momentum_tool,
...
# Tiered equity universe (``assets_config.json``) — injected into prompts so tools and copy stay aligned.
_core_eq = equity_core_tickers()
_CREW_FD_WATCHLIST_CN = financial_datasets_tickers_for_prompt()
_CREW_YF_ENUM = "／".join(ai_sector_yfinance_symbols())
_CREW_FD_CORE_PHRASE = (
    f"{_core_eq[0]}、{_core_eq[1]} 各≥2 行" if len(_core_eq) >= 2 else f"{_core_eq[0]} ≥2 行" if _core_eq else "NVDA、MSFT 各≥2 行"
)
_CREW_FD_CORE_BACKTICK = (
    f"`{_core_eq[0]}`、`{_core_eq[1]}` 各至少兩行"
    if len(_core_eq) >= 2
    else f"`{_core_eq[0]}` 至少兩行"
    if _core_eq
    else "`NVDA`、`MSFT` 各至少兩行"
)
...
_RISK_MODE_RULE = dedent("""\
    ...
    - **主判定為 neutral 時**：`portfolio_framing_summary` 與 **exec_summary** 嚴禁「同步做多偏好」「跨資產偏多配置」「維持跨資產偏多」等等同 **全面 risk_on** 的措辭；應改為「在 neutral 風險預算內之有限 Beta／不超總曝險上限」等與 **neutral** 一致之表述（組裝層亦會軟化常見片語）。
...
_GATE_VALIDATE_PICK_RULE = dedent("""\
    ...
    4) **加密理由**（純文字 ≥34 字）：須滿足下列**任一**——(a) 同句（或連續一段）內可讓系統辨識 **≥2 類**線索關鍵詞，建議從「新聞／催化／ETF／監管／鏈上／交易所／淨流／資金費率／多空比／OI／未平倉／現貨／清算／期貨／衍生品／CME／恐慌貪婪（恐懼貪婪）／**技術盤面（RSI、均線、MA20/MA50、超買超賣、多頭排列等）／宏觀讀值（DXY、VIX、美債殖利率、利差、SOFR）／儀表板估值讀值（NVT、Dominance）**」等任選**兩個不同概念**寫入；(b) **1 類**線索＋明確 **大型幣／流動性／退階／缺乏其他催化** 等退階語；(c) **1 類**線索且全文 **≥72 字**並**逐一點名** QSREC 內**每一檔**加密 asset（含比值如 BTC/SOL 須**同時出現 BTC 與 SOL** 字樣）。
