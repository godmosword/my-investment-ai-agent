    ...
    def run(self, exclude_context: str | None = None):
        ...
        final_report_task = Task(
            description=dedent(f"""
                ...
                ════ 嚴禁刪減！加密市場 3 則新聞 + 5 則推文必須完整保留！【資金流向與精準操作 (Crypto)】必須輸出！════
                ════ 嚴禁使用 HORIZON 標籤！嚴禁使用任何 Markdown 符號！════
            """),
            expected_output="戰報上半部：含標題列、今日市場模式、加密市場數據儀表板、3 幣圈新聞與 5 推文（每則僅含 IMPACT 標籤與 💎 主編共識）、【資金流向與精準操作 (Crypto)】1 單邊標的 + 1 配對交易，符合 Telegram HTML 格式。",
    ...
    def run(self, exclude_context: str | None = None):
        ...
        final_report_task = Task(
            description=dedent("""
                ...
                ════ 嚴禁使用 HORIZON 標籤！嚴禁使用任何 Markdown 符號！════
            """),
            expected_output="戰報下半部：AI 市場區塊（OpenRouter 模型熱度排名 + 基建/投資/模型各 3 新聞 + 5 推文，每則僅含 IMPACT 標籤與 💎 主編共識）、【AI 產業鏈精準操作 (US Equities)】2 支，符合 Telegram HTML 格式，不呈現個別 Agent 觀點。",
            agent=self.quant_strategist,
