...
_RISK_MODE_RULE = dedent("""\
    ...
    - 無論訊號是否衝突，必須在交易段落前輸出 1 行「訊號衝突摘要：...」（單行 ≤75 字；無衝突時寫「訊號衝突摘要：無顯著多空衝突。」；有衝突時只列 2~3 個關鍵矛盾點）。
    - 交易段落前必須新增 1 行「今日風險預算：...」（依 Regime 風險預算硬規則；單行、勿展開長篇）。""")
...
_HEDGE_FUND_BRIEF_RULE = dedent("""\
    ...
    - **必留關鍵字**（系統會驗證）：「今日風險預算」「訊號衝突摘要」「本日選擇理由」；內容各 **一行為主**，能短則短。
    - 「本日選擇理由：」**單行**為主；須點名當日 QSREC 同類標的，並滿足動態選幣／選股規則（加密理由常需 ≥72 字、美股常需 ≥80 字才能過關；用關鍵詞密佈、勿故事體）。
