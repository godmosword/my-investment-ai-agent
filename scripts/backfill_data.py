...
# BigQuery schema（與 bigquery_writer.extract_and_save_metrics 一致）
SCHEMA = [
...
def build_daily_dataframe(dxy_df: pd.DataFrame, mvrv_df: pd.DataFrame, days: int) -> pd.DataFrame:
...
def main() -> None:
    ...
    df = build_daily_dataframe(dxy_df, mvrv_df, days=args.days)
