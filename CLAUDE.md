...
- **機構 Phase D**（optional）: `STRICT_INSTITUTIONAL_PHASE_D_GATE=1` — 【昨日命題複盤】與【關鍵假設狀態】（各 2–4 條；狀態條數須與【關鍵假設】一致，且每條以（持續成立）／（部分動搖）／（已失效）開頭）；結構化要求 `prior_thesis_recap_lines`、`assumption_status_lines`。
