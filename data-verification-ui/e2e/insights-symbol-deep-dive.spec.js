...
test.describe("Insights symbol deep dive", () => {
  ...
  test("missing filing is 尚無本股財報摘要 without trailing UNKNOWN", async ({ page }) => {
    await page.goto("/insights?symbol=NVDA", { waitUntil: "load" });
    await expect(page.getByTestId("symbol-deep-dive")).toBeVisible({ timeout: 60_000 });
    await expect(page.getByTestId("symbol-filing-empty")).toBeVisible();
    await expect(page.getByTestId("symbol-filing-empty")).toHaveText("尚無本股財報摘要");
    await expect(page.getByTestId("symbol-filing-empty")).not.toContainText("UNKNOWN");
    await expect(page.getByTestId("symbol-filing-empty")).not.toContainText("。");
