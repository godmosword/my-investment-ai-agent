...
**其他專案接入 `/agent-plan`／`/agent-action`：** 見可攜模板 [`templates/agent-orchestration/`](../templates/agent-orchestration/)（Meta 通用 + [`AGENT-DOMAIN.template.md`](../templates/agent-orchestration/docs/AGENT-DOMAIN.template.md) 一頁填寫；Q-Silicon 填好範例見 [`AGENT-DOMAIN.example.q-silicon.md`](../templates/agent-orchestration/docs/AGENT-DOMAIN.example.q-silicon.md)）。一鍵複製：`templates/agent-orchestration/bootstrap.sh /path/to/your-repo`。
